...
def mask_to_qimage(mask: np.ndarray) -> QtGui.QImage:
    if mask is None:
        return QtGui.QImage()
    if mask.ndim == 2:
        h, w = mask.shape
        bytes_per_line = w
        return QtGui.QImage(mask.data, w, h, bytes_per_line, QtGui.QImage.Format_Grayscale8).copy()
    return cv2_to_qimage(mask)


class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('Dimension + Segmentation')
        self.resize(1200, 700)

        self.camera1 = Camera1(camera_index=0, marker_size_cm=5.0)
        self.camera2 = Camera2(camera_index=1, marker_size_cm=5.0)
        self.seg_model: SegmentationModel = None
        ...
        # Thread pool for background tasks (segmentation)
        self.executor = QtCore.QThreadPool.globalInstance()
    ...
    def _setup_ui(self):
        ...
        seg_group = QtWidgets.QLabel('Segmentation:')
        seg_group.setStyleSheet('font-weight: bold; font-size: 14px; margin-top: 10px;')
        ...
        controls.addWidget(self.segment_btn)
        ...
        # Segmented image
        controls.addWidget(QtWidgets.QLabel('Segmented Mask:'))
        self.mask_label = QtWidgets.QLabel()
        self.mask_label.setFixedSize(280, 210)
        self.mask_label.setStyleSheet('background: #111; border: 1px solid #444;')
        controls.addWidget(self.mask_label)
    ...
    def start_camera1(self):
        ...
        if ok:
            ...
            self.segment_btn.setEnabled(self.seg_model is not None)
        else:
    ...
    def load_model(self):
        path, _ = QFileDialog.getOpenFileName(self, 'Open model file', os.getcwd(), 'Models (*.h5 *.pt *.pth *.torchscript)')
        if not path:
            return
        try:
            # Infer framework automatically
            self.seg_model = SegmentationModel(path, framework='auto', input_size=(256, 256))
            self.seg_model.load()
            QMessageBox.information(self, 'Model', f'Loaded model: {os.path.basename(path)}')
    ...
    def run_segmentation(self):
        if self.seg_model is None:
            # Enable segmentation if camera1 is active
            if self.camera1.is_opened:
                self.segment_btn.setEnabled(True)
            QMessageBox.warning(self, 'Segmentation', 'Load a segmentation model first')
            return
        if not self.camera1.is_opened:
            QMessageBox.warning(self, 'Camera1', 'Camera 1 is not opened')
            return

        frame = self.camera1.read_frame()
        if frame is None:
            QMessageBox.warning(self, 'Segmentation', 'Failed to read frame from Camera 1')
            return

        # Run segmentation in a QRunnable to avoid blocking GUI
        runnable = SegmentationRunnable(self.seg_model, frame, self._on_segmentation_done)
        self.executor.start(runnable)

    def _on_segmentation_done(self, mask: np.ndarray):
        # show mask in mask_label
        if mask is None:
            QMessageBox.warning(self, 'Segmentation', 'Segmentation failed or returned no mask')
            return
        qimg = mask_to_qimage(mask)
        pix = QtGui.QPixmap.fromImage(qimg).scaled(self.mask_label.size(), QtCore.Qt.KeepAspectRatio)
...
class SegmentationRunnable(QtCore.QRunnable):
    def __init__(self, model: SegmentationModel, frame: np.ndarray, callback):
        super().__init__()
        self.model = model
    ...
    def run(self):
        try:
            mask = self.model.predict_mask(self.frame, threshold=0.5)
        except Exception:
            mask = None
        # invoke callback on main thread
        QtCore.QMetaObject.invokeMethod(
            QtWidgets.QApplication.instance(),
            lambda: self.callback(mask),
